...
class deployApplicationV2Test extends Specification {

    def namespace = 'six-baseimages-unittest'
    def gitRepo = '/opspoc/ocp-deployment-service.git'
    def gitTagDeploy = 'unittest/deploymentV2'
    ...
    def "deploy project"() {
        ...
        def deployApplicationV2 = pipeline.loadScript()

        when:
        deployApplicationV2(changeNo, namespace, gitRepo, gitTagDeploy, "", "")


        then:
        1 * openshift.withCluster(_) >> { Closure c -> c.call() }
        1 * openshift.withProject(_, _) >> { String s, Closure c -> c.call() }
        18 * openshift.selector(_, _) >> { String kind, String name -> // 2 times (undeploy and deploy ) for each component + 1 for rollout
        ...
        8 * openshift.apply(_) // 1 time for each component (excluding dc)
        8 * openshift.delete(_) // 1 time for each component (excluding dc)

        15 * selector.exists() >> true // 2 times (undeploy and deploy ) for each component
